...
struct BigInteger div1(struct BigInteger a, struct BigInteger b) {
    ...
    if (a.head==NULL||isZero(a)) {
        ...
        while (Cmp(aTemp,b)==1&&!isZero(aTemp)) {
            aTemp=sub(aTemp,b);
            quotient++;
        }
...
struct BigInteger mod(struct BigInteger a,struct BigInteger b)
{
    ...
    if (a.head==NULL||isZero(a)) {
        ...
        struct BigInteger rem;
        while (aTemp.sign==1&&Cmp(aTemp,b)==1&&!isZero(aTemp)) {
            aTemp=sub(aTemp,b);
            quotient++;
        }
    result=aTemp;
...
int Cmp(struct BigInteger a, struct BigInteger b) {
    int max=0;
...
struct node* multiply(struct node* num1, struct node* num2) {
    ...
    while (num2) {
        ...
        currentResult = NULL;
        for (int i = 0; i < pos; i++)
            insertAtBegin(&currentResult, 0); // Initialize with trailing zeros

        while (num1Copy) {
            int product = 0;
            product = (num1Copy->data) * (num2->data) + carry;
            carry = product / 10;
            insertAtEnd(&currentResult, product % 10); // Append the result to the currentResult
            num1Copy = num1Copy->next;
        }

        if (carry > 0) {
            insertAtEnd(&currentResult, carry); // If there is a carry, add it
        }
